...
public class FdDBAdapter {
	...
	public FdDBAdapter ( Context context){
		/* checking database version from server
		 * */
		int DATABASE_VERSION = 1;
		v_dbHelper = new fastDBHelper ( context, DATABASE_NAME, null, DATABASE_VERSION+2);
		_context = context;
	}
	...
	public void sync(){
		...
		try {
			...
			for (int i = 0; i < names.length(); i++) {
				JSONObject category = categories.getJSONObject(names.getString(i));
	...
	public Cursor getAllTables(){
		...
		//Cursor result = v_db.query(DATABASE_TABLE_TABLELIST, columns, null, null, null, null, null);
		return null;
	...
	public Cursor getAllCategories(){
		...
		Cursor result = v_db.query(DATABASE_TABLE_CATEGORIES, columns, null, null, null, null, null);
		return result;
	}
	...
	public Cursor getItems( String categoryFilter){
		if ( categoryFilter == null){
			return null;
		}
		else{
			String selection = CATEGORIES_KEY_ID + " = " + categoryFilter; 
			String[] columns = new String[] {"item_name","item_decription","price"};
			Cursor result = v_db.query( DATABASE_TABLE_ITEM, columns, selection, null, null, null, null);
			return result;
		}
	...
	private static class fastDBHelper extends SQLiteOpenHelper {
		/*** constructor**/
		public fastDBHelper(Context context, String name, CursorFactory factory, int version) {
