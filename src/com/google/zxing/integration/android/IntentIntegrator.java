 ...
 * 
 * <p>
 * Integration is essentially as easy as calling {@link #initiateScan(Activity)}
 * and waiting for the result in your app.
 * </p>
 * 
 * <p>
 * It does require that the Barcode Scanner application is installed. The
 * {@link #initiateScan(Activity)} method will prompt the user to download the
 ...
 * 
 * <p>
 * This is where you will handle a scan result. Second, just call this in
 * response to a user action somewhere to begin the scan process:
 * </p>
 * 
 * <pre>
 ...
 * or {@link #initiateScan(Activity, int, int, int, int)} to customize the
 * download prompt with different text labels.
 * </p>
 * 
 * <p>
 * Note that {@link #initiateScan(Activity)} returns an {@link AlertDialog}
 ...
 * {@link #shareText(Activity, CharSequence)}.
 * </p>
 * 
 * <p>
...
public final class IntentIntegrator {
	...
	public static final String ONE_D_CODE_TYPES = IntentIntegrator.PRODUCT_CODE_TYPES
	...
	public static AlertDialog initiateScan(Activity activity) {
		return IntentIntegrator.initiateScan(activity,
	...
	public static AlertDialog initiateScan(Activity activity, int stringTitle,
		...
		return IntentIntegrator.initiateScan(activity, activity
	...
	public static AlertDialog initiateScan(Activity activity,
		...
		return IntentIntegrator.initiateScan(activity, stringTitle,
	...
	public static AlertDialog initiateScan(Activity activity,
		...
		Intent intentScan = new Intent(IntentIntegrator.PACKAGE + ".SCAN");
		intentScan.setPackage(IntentIntegrator.PACKAGE);
		...
		try {
			...
			return IntentIntegrator.showDownloadDialog(activity, stringTitle,
					stringMessage, stringButtonYes, stringButtonNo);
		}
	}
	...
	/**
	 * See
	 * {@link #shareText(Activity, CharSequence, CharSequence, CharSequence, CharSequence, CharSequence)}
	 * -- same, but uses default English labels.
	 */
	 ...
	 */
	public static void shareText(Activity activity, CharSequence text,
			int stringTitle, int stringMessage, int stringButtonYes,
			int stringButtonNo) {
		IntentIntegrator.shareText(activity, text, activity
				.getString(stringTitle), activity.getString(stringMessage),
				activity.getString(stringButtonYes), activity
						.getString(stringButtonNo));
	}
	...
	public static void shareText(Activity activity, CharSequence text,
		...
		intent.setAction(IntentIntegrator.PACKAGE + ".ENCODE");
		intent.setPackage(IntentIntegrator.PACKAGE);
		intent.putExtra("ENCODE_TYPE", "TEXT_TYPE");
		intent.putExtra("ENCODE_DATA", text);
		try {
			activity.startActivity(intent);
		} catch (ActivityNotFoundException e) {
			IntentIntegrator.showDownloadDialog(activity, stringTitle,
					stringMessage, stringButtonYes, stringButtonNo);
		}
	}
